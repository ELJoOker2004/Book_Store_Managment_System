...
class Main(object):
    def __init__(self,master):
        ...
        #search button
        search_button = tk.Button(searchbar, text="Search", command=search)
        search_button.grid(row=0, column=1, padx=(0, 10), pady=5)
        ...
        for text, command in links:
            label = tk.Label(centerLeftFrame, text=text, fg="blue", cursor="arrow", bg="grey",width=10)
            label.pack(pady=5)
...
def main():
    ...
    # header_win = tk.Text(win,width=50,height=20)
    # header_txt = tk.Label(win, text= "WELCOME BACK")
    # header_txt.config(font=("Times New Roman", 48),pady= 30)
